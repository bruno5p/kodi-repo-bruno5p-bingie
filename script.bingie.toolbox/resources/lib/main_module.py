...
class MainModule:
    ...
    def setfont(self):
        '''allows the user to change the skin font by setting lookandfeel.font and reloading skin'''
        header = self.params.get("header", "Font")
        result = SkinSettings().set_skin_setting(setting="Font", window_header=header)
        if result:
            value, label = result
            if value is not None:
                font_name = value if value else "Default"
                xbmc.executeJSONRPC(
                    '{"jsonrpc":"2.0","id":1,"method":"Settings.SetSettingValue",'
                    '"params":{"setting":"lookandfeel.font","value":"%s"}}' % font_name
                )
